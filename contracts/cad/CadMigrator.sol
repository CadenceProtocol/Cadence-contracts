...
import "./interfaces/ICadIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/ICadMigrator.sol";

contract CadMigrator is ReentrancyGuard, ICadMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "CadMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "CadMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "CadMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "CadMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "CadMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "CadMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "CadMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "CadMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "CadMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "CadMigrator: token not whitelisted");
        require(_tokenAmount > 0, "CadMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "CadMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(cadPrice);
        require(mintAmount > 0, "CadMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "CadMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "CadMigrator: invalid tokenA");
            require(tokenB != address(0), "CadMigrator: invalid tokenB");
        ...
        ICadIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "CadMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "CadMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "CadMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "CadMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("CadMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "CadMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "CadMigrator: invalid _action");
