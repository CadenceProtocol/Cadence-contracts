...
contract Timelock is ITimelock {
    ...
    function initClpManager() external onlyAdmin {
        IClpManager _clpManager = IClpManager(clpManager);

        IMintable clp = IMintable(_clpManager.clp());
        clp.setMinter(clpManager, true);

        IUSDG usdg = IUSDG(_clpManager.usdg());
        usdg.addVault(clpManager);

        IVault vault = _clpManager.vault();
        vault.setManager(clpManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        IRewardRouterV2 _rewardRouter = IRewardRouterV2(rewardRouter);

        IHandlerTarget(_rewardRouter.feeClpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(_rewardRouter.stakedClpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(clpManager).setHandler(rewardRouter, true);
    }
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = IClpManager(clpManager).usdg();
        uint256 balance = IERC20(usdg).balanceOf(clpManager);

        IUSDG(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            IUSDG(usdg).mint(clpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IUSDG(usdg).burn(clpManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        IClpManager(clpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }
    ...
    function setClpCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        ...
        IClpManager(clpManager).setCooldownDuration(_cooldownDuration);
    }
