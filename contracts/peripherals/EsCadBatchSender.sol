...
contract EsCadBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esCad;

    constructor(address _esCad) public {
        admin = msg.sender;
        esCad = _esCad;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsCadBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esCad).transferFrom(msg.sender, _accounts[i], _amounts[i]);
